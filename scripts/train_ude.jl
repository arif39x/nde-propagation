...
function loss(p, _)
    ...
    if sol.retcode != ReturnCode.Success    # Return a high penalty if unstable to keep the optimizer away from bad zones
        return 1.0f7
    end
    ...
    data_loss = sum(abs2, pred_I .- observed_I)   # Mean Square Error

    reg_loss = 0.01f0 * sum(abs2, p.neural) #keeps weights small to prevent the ODE from exploding
...
println("Stabilizing with Adam (200 iterations)...")
...
title!("Malware Dynamics Discovery")
xlabel!("Time")
